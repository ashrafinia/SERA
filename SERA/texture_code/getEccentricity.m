...
sFactor = 1; % scaling factor
try
    mask = imresize3D(mask,[],[round(double(size(mask,1))),round(double(size(mask,2))),round(double(size(mask,3))*sFactor)],'nearest','fill');
catch
    warning(['Eccentricity: adding a third dimension to "mask" with size ',mat2str(size(mask)), ' and values of  ',mat2str(mask(:))]);
...
% ADDED BY SAEED: to prevent complex number:
% radii = sqrt(1 ./ abs(diag(evals)));
% radii = sqrt(1 ./ bsxfun(@max,diag(evals),0));
